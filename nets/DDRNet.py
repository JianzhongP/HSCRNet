...
class DualResNet(nn.Module):
    ...
    def __init__(self, block, layers, img_size=[256, 256], num_classes=19, planes=64, spp_planes=128, head_planes=128, augment=True):
    ...
    def forward(self, x):
        ...
        if self.augment: 
            ...
            x_ = F.interpolate(x_, size=(self.img[0], self.img[1]), mode='bilinear', align_corners=True)
            return x_      

def DualResNet_imagenet(img_size=[512, 512], num_classes=21):
    model = DualResNet(BasicBlock, [2, 2, 2, 2], img_size=img_size, num_classes=num_classes, planes=32, spp_planes=128, head_planes=7, augment=False)
    '''
    if pretrained:
