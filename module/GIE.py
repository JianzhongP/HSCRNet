...
class Mlp(nn.Module):
    ...
    def forward(self, x):       
        x = self.fc1(x)   #1*1 conv
        x = self.act1(x)  #activate_GELU
        x = self.drop(x)        
        x = self.conv(x)  #Res_DWconv    
        x = self.fc2(x)               
        x = self.drop(x)
        return x
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        #*********************************************************Ìí¼Ó
        attn = torch.zeros((B, self.num_heads, N, N))
...
class StokenAttention(nn.Module):
    ...
    def stoken_forward(self, x):
        ...
        #*************************************************************************Ìí¼Ó
        stoken_features = torch.zeros((B, C, hh, ww))
        stoken_features = stoken_features.type(torch.float32)
        pixel_features = torch.zeros((B, hh*ww, h*w, C))
        pixel_features = pixel_features.type(torch.float32)
        ...
        with torch.no_grad():
            for idx in range(self.n_iter):
                ...
                affinity_matrix = pixel_features @ stoken_features * self.scale # (B, hh*ww, h*w, 9)  @Îª¾ØÕó³Ë·¨
...
def test():
    model = STViT(embed_dim=[96, 192, 384], # 52M, 9.9G, 361 FPS
                    depths=[4, 6, 14],
                    num_heads=[2, 3, 6],
                    n_iter=[1, 1, 1], 
                    stoken_size=[4, 4, 4],
